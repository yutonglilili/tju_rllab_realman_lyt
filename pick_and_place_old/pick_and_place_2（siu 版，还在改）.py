...
2. 优化 moveit server 脚本，改为一次性规划好全部动作，全部规划成功再执行，而不是执行到一半退掉(目前拆成 pick 和 place 两个阶段后，这种情况有较大缓解，不急)；
...
# 保存图像路径
SAVE_DIR = "/home/zhangzhao/tvla-realenv/lyt/pick_and_place_2.0/"
...
def prepare_place(obs, obs_rs_dual, rs_env, cam_results, task, home_T_tcp2base, pick_targets, arm_name):
    ...
    while True:
        ...
        # 修正相机的标定问题，将 x 轴向左偏移 1cm
        place_Ttcp2base = make_lift_T(place_Ttcp2base, lift_x=0.015)

        place_Ttcp2base = make_lift_T(place_Ttcp2base, lift_z=0.07)
        ...
        # 将过渡点抬高 8cm
        intermediate_Ttcp2base = make_lift_T(intermediate_Ttcp2base, lift_z=0.08)
...
def generate_fallback_place_pose(place_targets, fallback_rx_deg=10):

    print("⚠ Using fallback place pose:", fallback_rx_deg)

    place_T = copy.deepcopy(place_targets["place_Ttcp2base"])

    rx = np.deg2rad(fallback_rx_deg)
        ...
        # -----------------
            ...
            if plan_success == False:
                ...
                for retry in range(max_retry):
                    print(f"🔁 planning attempt {retry+1}/{max_retry}")
                    new_place_targets = generate_fallback_place_pose(place_targets, fallback_rx_deg=10*retry)
                    action_seq, action_names = build_place_sequence(new_place_targets)
                    status, plan_success, failed_wp, T_res, G_res = send_action_sequence_to_web(action_seq, action_names, arm_name, WEB_BATCH_URL, BATCH_USE_MOVEIT, BATCH_SPEED_PCT, BATCH_BLEND_R_PCT, GRIPPER_OPEN, GRIPPER_CLOSE)
                    if plan_success:
                        break

                if plan_success == False:
                    print("🚫 Place failed after all retries")
                    return False
...
if __name__ == "__main__":
    ...
    instruction = "Pick the rubic's cube and place it on the blue plate, then pick the yellow ball and place it on the pink plate, then pick the brown horse and place it on the blue plate, then pick the white ball and place it on the pink plate, then pick the glue stick and place it on the pink plate, then pick the carrot and place it on the blue plate, then pick the iron cup and place it on the pink plate."
    # instruction = "Pick the glue stick and place it on the pink plate, then pick the carrot and place it on the blue plate, then pick the tiddy bear and place it on the pink plate."
    # instruction = "Pick the rubic's cube and place it on the blue plate, then pick carrot and place it on the pink plate."
