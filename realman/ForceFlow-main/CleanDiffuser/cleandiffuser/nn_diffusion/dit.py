...
class DiT1d(BaseNNDiffusion):
    ...
    def __init__(
        ...
        x_dim: int,
        x_seq_len: int,
        emb_dim: int = None,        # 保持向后兼容
        vec_emb_dim: int = None,    # vec_condition维度
        seq_emb_dim: int = None,    # seq_condition维度
        d_model: int = 384,
        n_heads: int = 6,
        depth: int = 12,
        attn_dropout: float = 0.0,
        ...
        # 简单处理参数
        if vec_emb_dim is None: vec_emb_dim = emb_dim
        if seq_emb_dim is None: seq_emb_dim = emb_dim
        # 如果emb_dim是None，使用vec_emb_dim作为timestep embedding的维度
        timestep_emb_dim = emb_dim if emb_dim is not None else vec_emb_dim
        
        super().__init__(timestep_emb_dim, timestep_emb_type, timestep_emb_params)
        # input projection
        self.x_proj = nn.Linear(x_dim, d_model)
        self.t_proj = nn.Sequential(
            nn.Linear(timestep_emb_dim, d_model), nn.SiLU(), nn.Linear(d_model, d_model)
        )
        self.cond_proj = nn.Sequential(nn.Linear(vec_emb_dim, d_model), nn.LayerNorm(d_model))
        if use_cross_attn:
            self.seq_cond_proj = nn.Sequential(nn.Linear(seq_emb_dim, d_model), nn.LayerNorm(d_model))
        else:
