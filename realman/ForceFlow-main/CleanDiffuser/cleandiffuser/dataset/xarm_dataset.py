...
class xArmDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 拼接6维位移动作 + 1维夹爪动作 + 6维delta_force = 13维
        action = np.concatenate((action_6d, gripper_action, delta_force), axis=-1)
        ...
        return {
            "obs": {
                "pos": torch.tensor(pos_with_gripper, dtype=torch.float32),  # 7维：6维位姿 + 1维夹爪状态
                "rgb_arm": torch.tensor(observation["rgb_arm"], dtype=torch.float32),
                "rgb_fix": torch.tensor(observation["rgb_fix"], dtype=torch.float32),
