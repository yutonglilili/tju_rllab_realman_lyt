"""
清桌面任务脚本（基于多线程 pick-and-place 执行器）

功能：
1. 不再由顶层指令显式列出每个 pick/place 物体。
2. 每轮先让模型读取当前图像，自动找出符合指令的任务，并输出物体-容器表格。
3. 逐个执行任务。
4. 一轮完成后再次用同一指令让模型判断是否完成；若未完成则继续下一轮，完成则退出。
"""
...
from multi_pointing_vllm_get_point_utils import (
    ...
    generate_task_table_from_scene,
...
def main():
    ...
    instruction = "Clear the table. Pick all toys and place them on the basket."
    ...
    try:
        while not is_task_completed:
            ...
            # 1. 获取当前图像
            image_rgb = rs_env.step()["rgb"]
            
            # 2. 生成任务表
            task_table = generate_task_table_from_scene(image_rgb, instruction)
            task_list = task_table["tasks"]

            # 3. 执行任务
            run_all_tasks(state, env, rs_env, cam_results, task_list, home_T_tcp2base)
            
            # 4. 检查任务是否完成
            is_task_completed = check_instruction_complete(image_rgb, instruction)
