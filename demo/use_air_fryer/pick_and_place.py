...
PLACE_Z_OFFSET = 0.07           # place 阶段 z 轴高度偏移（米）

MOVE_OBJECT_THRESHOLD = 0.05    # 物体移动检测阈值（米，5cm）
MOVE_CONTAINER_THRESHOLD = 1 # 容器移动检测阈值（米，）(废掉)
...
# 保存图像路径
SAVE_DIR = "/home/zhangzhao/lyt/demo/logs/save_images/"
    ...
    while not state.stop_all.is_set():
        ...
        if state.tracking_mode:
            ...
            try:
                ...
                # 保存打点图片
                # save_pointed_image(image_rgb, point_2d, save_dir=SAVE_DIR, prefix=target_name)
                ...
                # 由于空气炸锅的特殊性，需要修正 place 阶段的 y 轴
                if task_phase == TaskPhase.PLACE:
                    target_T = make_lift_T(target_T, lift_x=0.08, lift_y=-0.04)
...
def build_action_list(env, target_T, home_T_tcp2base, curobo_planner, task_phase):
    ...
    if task_phase == TaskPhase.PICK:
        ...
        pre_target_T = make_lift_T(target_T_new, lift_z=APPROACH_Z_OFFSET+0.03)
        pre_target_pose = realman_xyzrpy_from_T(pre_target_T)
        # 对于空气炸锅任务修改palce 的 rpy
        pre_target_pose[3:] = np.array([0.0623, 0.4881, 3.1218])
    ...
    if task_phase == TaskPhase.PICK:
        ...
        # 偏置
        target_T_new = make_lift_T(target_T_new, lift_z=-0.013)
        target_pose = realman_xyzrpy_from_T(target_T_new)
        target_gripper_state = GRIPPER_CLOSE

        post_target_T = make_lift_T(target_T_new, lift_y=APPROACH_Y_OFFSET, lift_z=APPROACH_Z_OFFSET+0.1)
        ...
        # 对于空气炸锅任务修改place 的 rpy
        target_pose[3:] = np.array([0.0623, 0.4881, 3.1218])
        ...
        # 对于空气炸锅任务修改post 的 rpy
        post_target_pose[3:] = np.array([0.0623, 0.4881, 3.1218])
